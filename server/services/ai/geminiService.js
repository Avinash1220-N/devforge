...
// Gemini 2.5 Flash pricing parameters (in USD per token)
const PRICE_INPUT_TOKEN = 0.075 / 1000000;  // $0.075 per million tokens
const PRICE_OUTPUT_TOKEN = 0.30 / 1000000;   // $0.30 per million tokens

/**
 * Calculates prompt token cost.
 */
function calculateCost(inputTokens, outputTokens) {
  return (inputTokens * PRICE_INPUT_TOKEN) + (outputTokens * PRICE_OUTPUT_TOKEN);
}
...
async function executeGeminiCall({ userId, feature, promptText, version, rawInput, isJson = true }) {
  ...
  // 1. Quota Check & Monthly Reset Automation
  const user = await User.findById(userId);
  if (user && user.aiBudget) {
    const currentMonth = new Date().toISOString().substring(0, 7); // "YYYY-MM"
    if (user.aiBudget.month !== currentMonth) {
      user.aiBudget.spent = 0;
      user.aiBudget.month = currentMonth;
      user.aiBudget.lastResetAt = new Date();
      await user.save();
      console.log(`[AI Quota] Budget auto-reset to $0 for user: ${userId} (${currentMonth})`);
    }

    if (user.aiBudget.spent >= user.aiBudget.limit) {
      console.warn(`[AI Quota Limit] User ${userId} has exceeded monthly budget: $${user.aiBudget.spent}/$${user.aiBudget.limit}`);
      throw new Error('Monthly AI budget quota exceeded. Please contact the administrator.');
    }
  }

  // 2. Generate SHA-256 cache key hash
  ...
  try {
    ...
    if (cached) {
      // Log cache hit usage (zero token cost)
  ...
  // 4. Handle Simulation Mode if Mock Key is configured
  if (isMockMode || !genAI) {
    ...
    const mockInputTokens = Math.round(promptText.length / 4);
    const mockOutputTokens = Math.round(JSON.stringify(simulated).length / 4);
    const mockCost = calculateCost(mockInputTokens, mockOutputTokens);

    // Update user budget
    if (user && user.aiBudget) {
      user.aiBudget.spent = (user.aiBudget.spent || 0) + mockCost;
      await user.save();
    }
    ...
    await AIUsage.create({
      userId,
      feature,
      tokensUsed: mockInputTokens + mockOutputTokens,
      responseTime: Date.now() - startTime,
      model: modelName,
      cacheHit: false
    });
  ...
  try {
    ...
    // Process cost
    const usageCost = calculateCost(promptTokens, candidateTokens);
    if (user && user.aiBudget) {
      user.aiBudget.spent = (user.aiBudget.spent || 0) + usageCost;
      await user.save();
    }
    ...
    // Resilient fallback: in local/development mode, if the real API fails, fall back to mock responses
    if (process.env.NODE_ENV !== 'production' || isMockMode) {
      console.log(`[Resilient Fallback] Gemini API call failed for '${feature}'. Returning simulated response.`);
      try {
        const simulated = getMockResponse(feature, rawInput);
        
        // Calculate mock fallback tokens
        const mockInputTokens = Math.round(promptText.length / 4);
        const mockOutputTokens = Math.round(JSON.stringify(simulated).length / 4);
        const mockCost = calculateCost(mockInputTokens, mockOutputTokens);
        ...
        console.error('Failed to generate mock response in fallback:', mockErr.message);
      }
    }
...
Passionated software engineer specializing in computer vision pipelines and interactive React web interfaces. Experienced in crafting intelligent full-stack solutions.
